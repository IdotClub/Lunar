...
use blackjack200\lunarlite\configuration\Punishment;
use blackjack200\lunarlite\GlobalBot;
use blackjack200\lunarlite\LunarLite;
use blackjack200\lunarlite\user\User;
use blackjack200\lunarlite\utils\Objects;
...
abstract class DetectionBase implements Detection {
	...
	final protected function format(string $fmt, string $message, bool $prefix = true) : string {
		...
		return sprintf(
			'%s%s',
			$prefix ? LunarLite::getInstance()->getPrefix() . ' ' : '',
	...
	public function fail(string $message) : void {
		...
		LunarLite::getInstance()->getScheduler()->scheduleTask(new ClosureTask(function (int $tick) use ($message) : void {
	...
	public function log(string $message) : void {
		...
		LunarLite::getInstance()->getLogger()->info($fmt);
		LunarLite::getInstance()->getDetectionLogger()->write($fmt);
	}
